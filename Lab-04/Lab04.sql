...
/*5)*/
SELECT MOV_ID,min(REV_STARS) AS MINIMUM_RATING FROM RATING group by MOV_ID order by MOV_ID desc;
/*6)*/
...
SELECT MOV_ID,MOV_TITLE,MAX(REV_STARS) FROM (
    ...
    (
        (SELECT MOV_TITLE,MOV_ID,MAX(REV_STARS) FROM MOVIE NATURAL JOIN RATING GROUP BY MOV_ID,MOV_TITLE HAVING COUNT(*)>=10) 
        NATURAL JOIN(
            DIRECTION 
            NATURAL JOIN
